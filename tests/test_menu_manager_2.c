...
/* Mocks */
static xcb_connection_t dummy_conn;
static xcb_ewmh_connection_t dummy_ewmh;
...
static void test_creation_and_destruction() {
  MenuManager *mgr = menu_manager_create(&dummy_conn, &dummy_ewmh);
  assert(mgr != NULL);
...
static void test_register_and_find() {
  ...
  cfg.title = "menu-1";
  Menu *menu = menu_create(NULL, &cfg);
...
static void test_key_event_activation() {
  MenuManager *mgr = menu_manager_create(&dummy_conn, &dummy_ewmh);
  MenuConfig cfg = menu_config_default();
  cfg.title = "menu-key";
