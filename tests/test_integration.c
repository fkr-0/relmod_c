...
static MockX11 setup_mock_x11(void) {
  MockX11 mock = {0};
  // Connection logic (including retries) is now handled by
  // input_handler_setup_x We still need a basic connection here for EWMH setup,
  // but rely on the handler setup for robustness.
  ...
  assert(mock.conn != NULL && !xcb_connection_has_error(mock.conn) &&
         "Initial mock connection failed");
