...
static bool terminating_handler(InputHandler *handler,
                                xcb_key_release_event_t *ev) {
  uint16_t new_state = ev->state;
  uint8_t released_key = ev->detail;
  uint16_t mask = handler->modifier_mask;
  if ((new_state == 0x40 || mask == 0x40) && released_key == 133) {
    printf("[DEBUG] Super key released - Terminating\n");
    return true;
  }
  if ((new_state == 0x08 || mask == 0x08) && released_key == 64) {
    printf("[DEBUG] Meta key released - Terminating\n");
    return true;
  }

  if ((new_state == 0x04 || mask == 0x04) && released_key == 37) {
    printf("[DEBUG] Ctrl key released - Terminating\n");
    return true;
  }
  if ((new_state == 0x01 || mask == 0x01) && released_key == 50) {
    printf("[DEBUG] Shift key released - Terminating\n");
    return true;
  }
  /* if (new_state == 0) { */
  /*   printf("All modifiers released\n"); */
  /* } */
  return false;
}
...
static bool handle_event(InputHandler *handler, xcb_generic_event_t *event) {
  bool exit = false;
  printf("[DEBUG] mask: 0x%x | Received event type: 0x%x\n",
         handler->modifier_mask, event->response_type & ~0x80);
  ...
  case XCB_KEY_PRESS: {
    ...
    if (handler->modifier_mask == 0) {
      uint32_t mod_mask = kp->state;
      uint8_t key = kp->detail;
      if (mod_mask != 0) {
        handler->modifier_mask = mod_mask;
        printf("[DEBUG] !!! Modifier mask set to 0x%x\n",
               handler->modifier_mask);
      } else if (key == 133) {
        handler->modifier_mask = 0x40;
        printf("[DEBUG] !!! Super - Modifier mask set to 0x%x\n",
               handler->modifier_mask);
      } else if (key == 64) {
        handler->modifier_mask = 0x08;
        printf("[DEBUG] !!! Meta - Modifier mask set to 0x%x\n",
               handler->modifier_mask);
      } else if (key == 37) {
        handler->modifier_mask = 0x04;
        printf("[DEBUG] !!! Ctrl - Modifier mask set to 0x%x\n",
               handler->modifier_mask);
      } else if (key == 50) {
        handler->modifier_mask = 0x01;
        printf("[DEBUG] !!! Shift - Modifier mask set to 0x%x\n",
               handler->modifier_mask);
      }
    }
  ...
  case XCB_KEY_RELEASE: {
    ...
    if (handler->modifier_mask == 0) {
      uint32_t mod_mask = kr->state;
      if (mod_mask != 0) {
        handler->modifier_mask = mod_mask;
        printf("[DEBUG] !!! Release: Global Modifier mask set to 0x%x\n",
               handler->modifier_mask);
      }
    }
    if (terminating_handler(handler, kr)) {
      exit = true;
      break;
    }
    for (size_t i = 0; i < handler->menu_count; i++) {
