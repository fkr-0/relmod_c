...
void cairo_menu_animation_init(CairoMenuData *data) {
    ...
    data->anim.hide_animation =
        menu_animation_fade_out(150); /* 150ms fade out */
...
void cairo_menu_animation_update(CairoMenuData *data, Menu *menu,
    ...
    if (data->anim.is_animating) {
        if (menu->state == MENU_STATE_INITIALIZING) {
            if (data->anim.show_sequence) {
                menu_animation_sequence_update(data->anim.show_sequence,
                                               delta_time);
            ...
            if (data->anim.hide_sequence) {
                menu_animation_sequence_update(data->anim.hide_sequence,
                                               delta_time);
...
void cairo_menu_animation_set_default(CairoMenuData *data,
    ...
    case MENU_ANIM_SLIDE_DOWN:
        data->anim.show_animation =
            menu_animation_slide_in(show_type, duration);
        break;
    case MENU_ANIM_ZOOM:
    ...
    case MENU_ANIM_SLIDE_DOWN:
        data->anim.hide_animation =
            menu_animation_slide_out(hide_type, duration);
        break;
    case MENU_ANIM_ZOOM:
...
void cairo_menu_animation_set_sequence(CairoMenuData *data, bool is_show,
