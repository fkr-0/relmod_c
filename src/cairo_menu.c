...
// Forward declaration for the static cleanup function
...
void menu_setup_cairo(xcb_connection_t *conn, xcb_window_t parent,
    ...
    menu->cleanup_cb = cairo_menu_cleanup; // Assign the static cleanup function
...
/* Cleanup resources */
// Keep original name and static linkage, matches cleanup_cb signature
static void cairo_menu_cleanup(void *user_data) {
...
// Public API function to destroy Cairo-specific resources associated with a
// menu. This function matches the declaration in cairo_menu.h.
void cairo_menu_destroy(Menu *menu) {
    if (!menu || !menu->user_data) {
        LOG("cairo_menu_destroy called with NULL menu or user_data");
        return;
    }

    // Check if it's likely CairoMenuData before casting and cleaning.
    // This relies on menu_cairo_is_setup which just checks user_data != NULL.
    // A more robust type check might be needed in a complex system.
    if (menu_cairo_is_setup(menu)) {
        LOG("Calling internal cairo_menu_cleanup for menu: %s",
            menu->config.title ? menu->config.title : "(untitled)");
        cairo_menu_cleanup(menu->user_data);
        menu->user_data = NULL; // Prevent double free by menu_destroy
    } else {
        LOG("cairo_menu_destroy called, but menu doesn't appear to have Cairo "
            "setup (user_data might be wrong type or NULL)");
    }
    // Note: This function specifically cleans up the Cairo resources
    // (user_data). The core Menu struct and its config items are freed by
    // menu_destroy in menu.c.
}
